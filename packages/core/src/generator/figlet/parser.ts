...
function parseCodePoint(token: string): number | null {
	...
	if (token.startsWith("0") && token.length > 1) {
		const n = parseInt(token.slice(1), 8);
		return Number.isNaN(n) ? null : n;
	}
...
function parseFigChar(
	...
	for (let i = 0; i < height; i++) {
		...
		const stripped =
			i === height - 1
				? stripEndmark(raw, endmark, 2)
				: stripEndmark(raw, endmark, 1);
		rows.push(stripped);
	}
...
export function parseFont(flfString: string): FigFont {
	...
	if (!headerLine.startsWith("flf2a")) {
		throw new Error(`Invalid FLF file: expected "flf2a" signature, got "${headerLine.slice(0, 10)}"`);
	}
	...
	const baseline = parseInt(baselineStr ?? "0", 10);
	const maxWidth = parseInt(maxWidthStr ?? "0", 10);
	const oldLayout = parseInt(oldLayoutStr ?? "0", 10);
	const commentLines = parseInt(commentLinesStr ?? "0", 10);
