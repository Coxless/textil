import type { AsciiGrid, AsciiGridJson, Cell } from "@textil/core";
...
export function parseGridFromJson(json: AsciiGridJson): AsciiGrid {
  const cells = json.rows.map((row, r) =>
    [...row].map((char, c) => {
      const color = json.colors?.[r]?.[c];
      return color ? { char, ...color } : ({ char } as Cell);
    }),
  );
...
export function parseGridFromPlainText(text: string): AsciiGrid {
  ...
  const cells = lines.map((line) => [...line].map((char) => ({ char }) as Cell));
